...
Instead, how about we filter the data once, save it into an object, and call 
on that object:
...
Better even than `apply()` or `map()` is not to iterate at all! Writing vectorized
functions is tough, but do-able in many cases. Here's an example:
...
30-seconds to run. Should you really spend your efforts making the subfunction take only 10-seconds? 
...
b.  A sum is a smaller number than a mean, so it needs less memory.
