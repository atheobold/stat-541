...
- Instead of ":memory:" inside your `dbConnect()` function, you should specify the path to your chinook.db file including the filename itself.
  * You can use `here()` to navigate to your database! 
:::
...
:::{.callout-tip}
You might find the `dbReadTable()` function helpful!
:::

2. How many different job titles are included in the `employees` table?

3. What is the average of the `Total` variable in the `invoices` table? **Round your answer to two decimal places!**
...
4. What two packages did the tutorial make use of?
...
5. What type of request will we likely start with when getting data from an API?
...
6. If `GET()` successfully requested information from the API, what `status` should you see in the response object?

7. How many people are currently aboard the International Space Station?
